...
> **直播流提示:**<br>如果使用的方案2，HA将无法直接访问直播流，你可以尝试在HA或者路由器上增加路由解决。<br>如果网络通常，但无法正常添加直播流，也可能时密码错误导致，建议HA配置时关闭直播流。
...
进入 **米家App -> 我的 -> 连接其他平台 -> 添加**，选择 `巴法`，输入你的账号密码关联米家并同步 `巴法云` 上的订阅频道。
...
方案2已详细描述接入方案，如果你使用的方案1，则需要通过bamfa集成将本地实体接入到米家，并参考方案2支持米家。
